...
browser read and its `gather_auth` routing, the Gmail read, and the
instruments-mining flow. The CLI shorthand throughout:
...
4. Run the [instruments-mining flow](gather.md#instruments-mining-gmail)
...
5. Write at the main level, per record, no form round-trip — the main
   ...
   from step 4. The explicit request plus the Touch ID tap are the
...
6. Report the deltas, old value to new: per program, per bank, and per
   instrument. Mark Gmail-derived numbers as stale statement hints,
   call out expired instruments, and flag everything
   `$CLI prefs instrument-list --expiring-within 90d` returns.
