...
| `enhance` | Background enhancer primitives: `targets <slug> <verify\|seat-advice>` (the enumerated worklist), `merge <slug> <verify\|seat-advice>` (concurrency-safe result upsert, JSON array on stdin) |
...
- Interactive surfaces stay at the main level: cc-present boards and `AskUserQuestion`. One artifact per trip: the intake, finalist, head-to-head, and booking phases are rounds on a single board, never a second artifact mid-trip (see [references/boards.md](references/boards.md)).
...
4. Gather the ask on the intake board: open the trip artifact's intake round per [references/boards.md](references/boards.md) — context, destination buckets, window, cabin, party, scope, and constraints as built-in blocks seeded from `prefs show`, drained on submit with the onboard form discipline. A typed hard-constraint field lands in `constraints` with `confirmed: true` in one hop. A hard deadline becomes a `constraints` entry only after the user confirms it — everything else is a preference. Never re-ask a pinned constraint. When the board is down, fall back to one `AskUserQuestion` (at most 4 questions, concrete options each) per [references/planning.md](references/planning.md), "When the board is down".
...
10. Kick off background enhancers per [references/enhancers.md](references/enhancers.md): `$CLI enhance targets <slug> verify` enumerates rows worth a live check — unknown seat sufficiency, stale cached reads, unverified empty leads — and `$CLI enhance targets <slug> seat-advice` enumerates the picks' equipment for a seat-map read. When targets exist, group by auth lane, prime the cookie grant only if step 7's tap didn't cover it, spawn both the verifiers and the seat-advice researchers as background subagents (sonnet — mechanical search-and-extract) in one message, and move on: the board never waits for them. On each completion notification, fold — `$CLI rank <slug> && $CLI trip finalize <slug>` — and annotate the live board per [references/planning.md](references/planning.md), "The board flow": "verified live HH:MM" beside the freshness stamp, live seat picks under the award segment, a prominent gone-on-`<host>` flag on a killed row. The row stays visible, and when it was the user's pick, tell them immediately.
11. Run the board through its phases per [references/boards.md](references/boards.md): push the **Finalists** round — ranked journeys, notable stretches, unpaired leads, search states, stays, one evidence line per active factor, every preference miss named. On the shortlist submit, drain and transition — two or more picks open a **Head to head** round comparing them side by side while verifiers annotate live, exactly one pick skips straight to the **Booking sheet**. The lock-in submit pushes the content-only booking sheet: every leg, transfer, tax, and seat pick for the journey. Each transition is `outcomes --no-doc`, apply the writes, `round --title`, then one full push; inside a round, only `update-block`/`reply`/`remove-block`.
12. Log every decision as it lands: `$CLI trip log <slug> "picked the CPT stitch; QR over EK for the DOH stop"`. Pin new constraints with `trip set` the moment they're pinned, mid-planning, never at wrap-up. Route always-true facts ("never through IST", a balance correction) to `prefs set`/`set-balance`/`set-status`/`instrument-add` right then, and API quirks to `learnings add --scope api`; the plugin's Stop hook (`hooks/reflect.py`) backstops the sweep at session end. When the user confirms the booking or calls the trip done, close the artifact: `cc-present close --summary "<journey one-liner>; sheet delivered"`.

## References

- [references/planning.md](references/planning.md) — parsing the ask, origin expansion, region sweeps, season awareness, affordability, journeys, routing shapes, hotels, and the board flow.
- [references/boards.md](references/boards.md) — the four board shapes: envelopes, blocks, id namespaces, and the phase-transition recipe.
- [references/workflows.md](references/workflows.md) — authoring an ad-hoc walker over the compiled graph.
- [references/enhancers.md](references/enhancers.md) — fire-and-forget background enhancers (`verify` and `seat-advice`): the contract, auth lanes, and the fold.
